...
for tag in tags:
    for epochs in epochss:
        ...
        try:
            ...
            train_examples = []
            f = open('train/' + tag + '_train_data_' + str(train_count+test_count) + '.csv','r', encoding='utf-8')
            rdr = csv.reader(f)
            for row in rdr:
            ...
            model_state_dict = torch.load("fineTunedModel/" + tag + "/" + str(tag) + "_epochs_" + str(epochs) + "_batchsize_" + str(batch_size) +  "_dataset_"  + str(train_count+test_count) +  ".pt", map_location=device)
            model.load_state_dict(model_state_dict, strict=False)

            print("check model")

        except FileNotFoundError:
            print("No file related to " + "tag " + tag + ", epochs " + str(epochs) + ", batchsize " + str(batch_size) +  ", dataset "  + str(train_count+test_count))
            continue
        ...
        resultFile.write("tag " + tag + ", epochs " + str(epochs) + ", batchsize " + str(batch_size) +  ", dataset "  + str(train_count+test_count) + '\n')
        resultFile.write("acc " + str(acc) + '\n')
        resultFile.write("hitsAt3 " + str(hitsAt3) + '\n')
        resultFile.write("hitsAt5 " + str(hitsAt5) + '\n')
        resultFile.write("hitsAt10 " + str(hitsAt10) + '\n')
        resultFile.write("rankingBasedMetric " + str(rankingBasedMetric) + '\n')
        resultFile.write('-------------------------------------------------\n')
